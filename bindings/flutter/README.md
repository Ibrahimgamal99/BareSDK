...
| | |
|---|---|
| **Android** | arm64-v8a · armeabi-v7a · x86_64 · `minSdk` 24. Prebuilt `libvoxsdk.so` ships in the package; Kotlin shim handles cache dir, audio focus, speakerphone, network callbacks, and an app-owned audio device (`AudioRecord`/`AudioTrack` on `VOICE_COMMUNICATION`). |
| **iOS** | 13.0+, device + simulator. Vendors `VoxSDK.xcframework`; Swift shim handles `AVAudioSession`, speaker routing, `NWPathMonitor`. Capture uses VoiceProcessingIO (hardware AEC), also for the optional app-owned audio device. |
| **Linux / macOS / Windows** | Works as a plain FFI binding — the app bundles the native library itself, or passes `VoxSDK(libPath: ...)`. |
...
The vendored `bindings/flutter/ios/Frameworks/VoxSDK.xcframework` is produced by
...
`VoxSDK.start()` is what makes the package portable across apps: it asks the
...
The native stack is a **process-wide singleton** — a second `VoxSDK` in a
process that already has one throws unless you `shutdown()` first, and
`VoxSDK.start()` reattaches (`sdk.reattached`) when a background isolate wakes
up against a live stack.

See [`example/`](example/) for a runnable app and
[`docs/quickstart/flutter.md`](https://github.com/Ibrahimgamal99/VoxSDK/blob/main/docs/quickstart/flutter.md)
