...
/// `dist/linux/x86_64/voxsdk.so`. Skipped automatically when missing.
///
/// Run: flutter test test/vox_sdk_smoke_test.dart
...
void main() {
  final libPath = File('../../dist/linux/x86_64/voxsdk.so').absolute.path;
  ...
  }, skip: haveLib ? false : 'dist/linux/x86_64/voxsdk.so not built');
  ...
  test('reattaches to a stack this isolate did not start', () async {
    ...
    final second = VoxSDK(
      config: const VoxSDKConfig(netMonitorIntervalSeconds: 0),
      libPath: libPath,
    );
    expect(second.reattached, isTrue,
        reason: 'construction on a live stack did not reattach');
    expect(VoxSDK.instance, same(second));
    ...
    final sdkEvents = <VoxSDKEvent>[];
    final acctEvents = <VoxSDKEvent>[];
    ...
    second.detach();
    expect(() => VoxSDK(libPath: libPath, reattachIfRunning: false),
        throwsStateError);

    // Reattach once more — this time to tear the stack down for real.
    final third = VoxSDK(libPath: libPath);
