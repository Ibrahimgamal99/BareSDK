...
/// Flutter app uses (real libvoxsdk, owned events, Dart decode).
///
...
void main() {
  ...
  test('UDP + WSS registration, echo call, media stats', () async {
    if (!Platform.isLinux || !File(libPath).existsSync()) {
      markTestSkipped('needs dist/linux/x86_64/voxsdk.so');
      return;
    }
    ...
    // always fire.
    Future<T> firstEvent<T extends VoxSDKEvent>(
        Stream<VoxSDKEvent> stream, bool Function(T) match,
