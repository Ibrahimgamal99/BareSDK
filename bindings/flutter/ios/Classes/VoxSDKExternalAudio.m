 ...
 * are exported by the dylib and resolved through -framework VoxSDK. */
extern int  voxsdk_audio_external_push(const int16_t *pcm, size_t nsamp);
extern int  voxsdk_audio_external_pull(int16_t *pcm, size_t nsamp);
extern int  voxsdk_audio_external_format(uint32_t *srate, uint8_t *ch,
                                          uint32_t *ptime);
extern bool voxsdk_audio_external_is_active(void);
...
@implementation VoxSDKAudioEngine
...
{
	VoxSDKAudioEngine *self = (__bridge VoxSDKAudioEngine *)inRefCon;
	OSStatus err;
	...
	err = AudioUnitRender(unit, ioActionFlags, inTimeStamp, 1,
	                      inNumberFrames, mic);
	if (err == noErr) {
		voxsdk_audio_external_push((const int16_t *)mic->mBuffers[0].mData,
		                            inNumberFrames * ch);
	}

	/* Playback: always fills, silence when no call is up, so there is nothing
	 * to branch on and no need to zero ioData ourselves. */
	voxsdk_audio_external_pull((int16_t *)ioData->mBuffers[0].mData,
	                            inNumberFrames * ch);
...
- (instancetype)init
{
	self = [super init];
	if (self) {
		_queue = dispatch_queue_create("dev.voxsdk.appaudio",
...
- (NSDictionary<NSString *, id> *)status
{
	...
	dispatch_sync(self.queue, ^{
		out = @{
			 ...
			 *
			 * available: on Android this is whether libvoxsdk.so loaded. iOS
			 * resolves the same symbols at link time through
			 * -framework VoxSDK, so reaching this line at all means they are
...
- (void)tick
{
	...
	uint8_t ch = 0;
	int err = voxsdk_audio_external_format(&srate, &ch, &ptime);
	...
	if (!self.running) {
		...
		NSLog(@"[VoxSDK] audio format changed %u/%u/%u -> %u/%u/%u",
                    ...
                    ptime:(uint32_t)ptime
{
	...
	if (!comp) {
		 ...
		 * (Android reports it when libvoxsdk.so did not load). */
	...
	self.running = YES;
	NSLog(@"[VoxSDK] app-owned audio open: %u Hz %u ch ptime=%u ms (VPIO)",
	      srate, ch, ptime);
}
 ...
 * lost from the logs. */
static NSString *VoxSDKPublicErrorCode(NSString *stage)
{
...
{
	[self teardownUnit];
	[self reportCode:VoxSDKPublicErrorCode(code)
...
- (void)mediaServicesReset:(NSNotification *)note
{
	...
	dispatch_async(self.queue, ^{
		NSLog(@"[VoxSDK] media services reset — rebuilding audio unit");
		[self teardownUnit];
	});
}
...
{
	NSLog(@"[VoxSDK] app-owned audio %@: %@", code, message);
