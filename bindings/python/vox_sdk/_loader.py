"""
_loader.py — cffi FFI setup for VoxSDK.

Reads _voxsdk_clean.h (generated from include/voxsdk.h by build.sh) so
the Python binding stays in sync with the C header automatically.

The library is searched in this order:
  1. VOXSDK_LIB environment variable (absolute path to .so/.dylib/.dll)
...
# Hard runtime deps on Linux — these are DT_NEEDED entries in voxsdk.so.
...
# VoxSDK handles it via dlopen; missing = AEC unavailable, not a crash.
...
# voxsdk.dll is built with x64-windows-static-md — OpenSSL and zlib are
...
def _check_linux_deps():
    ...
    if missing:
        raise ImportError(
            "VoxSDK: missing required system libraries.\n\n"
            + "\n".join(missing)
        )
...
def _check_windows_deps():
    ...
    if missing:
        raise ImportError("VoxSDK: missing required runtime.\n\n" + "\n\n".join(missing))
...
ffi = FFI()
with open(os.path.join(_here, "_voxsdk_clean.h")) as _f:
    ffi.cdef(_f.read())
...
def _dist_candidates():
    ...
    if sys.platform == "win32":
        yield os.path.join(_repo_root, "dist", "windows", "x64", "voxsdk.dll")
    elif sys.platform == "darwin":
        yield os.path.join(_repo_root, "dist", "macos", "universal", "voxsdk.dylib")
    else:
        arch = "arm64" if machine in ("aarch64", "arm64") else "x86_64"
        yield os.path.join(_repo_root, "dist", "linux", arch, "voxsdk.so")


def _find_lib():
    # 1. Explicit override
    env = os.environ.get("VOXSDK_LIB")
    if env:
        return env
