/**
 * voxsdk.hpp — header-only C++17 RAII wrapper around the VoxSDK C API.
 *
 * Self-contained: embeds the C declarations from voxsdk.h so you only need
 * this one header + the compiled shared library (voxsdk.so / .dll / .dylib).
 *
 * If you have already included voxsdk.h before this header, the embedded
 ...
 *
 *   VoxSDK::SDK sdk;
 *   sdk.config().transport = VOXSDK_TRANSPORT_TLS;
...
#ifndef VOXSDK_H
/**
 * @file voxsdk.h  VoxSDK public API
 *
 * Single header for all platforms. Include this and link voxsdk.a.
 ...
 *   - voxsdk_config_t carries version + struct_size for forward compat.
...
#ifdef _WIN32
#  ifdef VOXSDK_SHARED_BUILD
#    define VOXSDK_EXPORT __declspec(dllexport)
#  elif defined(VOXSDK_SHARED)
#    define VOXSDK_EXPORT __declspec(dllimport)
#  else
#    define VOXSDK_EXPORT
#  endif
#else
#  define VOXSDK_EXPORT __attribute__((visibility("default")))
#endif
...
#define VOXSDK_VERSION_MAJOR 1
#define VOXSDK_VERSION_MINOR 0
#define VOXSDK_VERSION_PATCH 0

VOXSDK_EXPORT const char *voxsdk_version(void);
VOXSDK_EXPORT const char *voxsdk_strerror(int err);
...
typedef enum {
	VOXSDK_CODEC_OPUS = 0,
	VOXSDK_CODEC_PCMU,     /* G.711 µ-law */
	VOXSDK_CODEC_PCMA,     /* G.711 A-law */
	VOXSDK_CODEC_G722,     /* deprecated — not compiled in */
	VOXSDK_CODEC_G726_32,  /* deprecated — not compiled in */
} voxsdk_codec_t;
 ...
 * preserving voxsdk_config_t memory layout.
 *
 * VOXSDK_AEC_SUPPRESSOR (default): built-in half-duplex TX suppressor.
 ...
 * VOXSDK_AEC_WEBRTC: full-duplex WebRTC acoustic echo cancellation.
 *   Desktop only; requires VOXSDK_WITH_WEBRTC_AEC build option and
 *   libwebrtc-audio-processing-1.  Returns ENOTSUP on mobile builds and
 *   when the option is off.
 */
/* VOXSDK_NO_PACKED_ENUM: for binding generators (e.g. ffigen) that ignore
...
#if defined(_MSC_VER) || defined(VOXSDK_NO_PACKED_ENUM)
typedef uint8_t voxsdk_aec_mode_t;
#  define VOXSDK_AEC_OFF        ((voxsdk_aec_mode_t)0)
#  define VOXSDK_AEC_SUPPRESSOR ((voxsdk_aec_mode_t)1)
#  define VOXSDK_AEC_WEBRTC     ((voxsdk_aec_mode_t)2)
#else
typedef enum __attribute__((packed)) {
	VOXSDK_AEC_OFF        = 0,
	VOXSDK_AEC_SUPPRESSOR = 1,
	VOXSDK_AEC_WEBRTC     = 2,
} voxsdk_aec_mode_t;
#endif
...
typedef enum {
	VOXSDK_DTMF_RFC4733  = 0, /* RFC 4733 RTP telephony-event (default) */
	VOXSDK_DTMF_SIP_INFO = 1, /* SIP INFO application/dtmf-relay */
	VOXSDK_DTMF_AUTO     = 2, /* prefer RFC 4733, fall back to SIP INFO */
} voxsdk_dtmf_mode_t;

typedef enum {
	VOXSDK_JBUF_ADAPTIVE = 0, /* adaptive jitter buffer (default) */
	VOXSDK_JBUF_FIXED    = 1, /* fixed-depth jitter buffer */
} voxsdk_jbuf_type_t;
...
typedef enum {
	...
	VOXSDK_ERR_INVAL                = -1,
	VOXSDK_ERR_NOMEM                = -2,
	VOXSDK_ERR_STATE                = -3,  /* wrong lifecycle state */
	VOXSDK_ERR_DNS                  = -4,
...
typedef enum {
	...
	VOXSDK_PRESENCE_OPEN,     /* available */
	VOXSDK_PRESENCE_CLOSED,   /* offline / DND */
	VOXSDK_PRESENCE_BUSY,     /* on a call */
} voxsdk_presence_status_t;

typedef enum {
	VOXSDK_100REL_DISABLED = 0, /* never send/require 100rel */
	VOXSDK_100REL_ENABLED  = 1, /* support 100rel if peer offers */
	VOXSDK_100REL_REQUIRED = 2, /* require 100rel; reject if unsupported */
 ...
 */
typedef enum {
	VOXSDK_PUSH_PROVIDER_NONE         = 0, /* no push params (default) */
	VOXSDK_PUSH_PROVIDER_APNS         = 1, /* Apple APNs production    */
	VOXSDK_PUSH_PROVIDER_APNS_SANDBOX = 2, /* Apple APNs development   */
	VOXSDK_PUSH_PROVIDER_FCM          = 3, /* Firebase Cloud Messaging  */
 ...
 */
typedef enum {
	VOXSDK_NET_CHANGE_DETECTED = 0,   /* local address set changed          */
	VOXSDK_NET_DOWN,                  /* no usable local address            */
	VOXSDK_NET_UP,                    /* usable local address again         */
	VOXSDK_NET_TRANSPORT_RESET,       /* SIP transports flushed + re-bound  */
	VOXSDK_NET_REREGISTERING,         /* REGISTER re-sent on the new path   */
	VOXSDK_NET_CALL_MIGRATING,        /* re-INVITE sent to move the media   */
	VOXSDK_NET_CALL_MIGRATE_ACCEPTED, /* peer answered; awaiting audio      */
	VOXSDK_NET_CALL_MIGRATED,         /* RTP confirmed on the new path      */
	VOXSDK_NET_CALL_MIGRATION_FAILED, /* gave up; see `error`               */
	VOXSDK_NET_CALL_DEFERRED,         /* not refreshable yet; will retry    */
	VOXSDK_NET_HANDOVER_FAILED,       /* transport reset failed; retrying   */
...
typedef struct {
	...
	voxsdk_call_handle_t    call;     /* CALL_* events only; else NULL    */
	voxsdk_account_handle_t account;  /* REREGISTERING only; else NULL    */
	 ...
	 */
	bool                     ice;
	voxsdk_error_t          error;    /* VOXSDK_OK unless *_FAILED       */
} voxsdk_ev_network_t;
...
typedef struct {
	...
	voxsdk_error_t          error;         /* VOXSDK_OK when REGISTERED */
...
typedef struct {
	voxsdk_account_handle_t account;
	voxsdk_call_handle_t    call;          /* call receiving the REFER */
...
typedef struct {
	voxsdk_account_handle_t account;
	voxsdk_call_handle_t    call;    /* the call we tried to transfer away */
 ...
 */
VOXSDK_EXPORT void voxsdk_event_release(const voxsdk_event_t *ev);
...
#define VOXSDK_CONFIG_VERSION 1

typedef struct {
	/* Forward-compat guard — MUST be set by voxsdk_config_init(). */
	uint32_t  version;      /* VOXSDK_CONFIG_VERSION */
	size_t    struct_size;  /* sizeof(voxsdk_config_t) at compile time */
	...
	/* ── Audio processing ─────────────────────────────────────────── */
	voxsdk_aec_mode_t aec_mode;  /* echo cancellation backend; default SUPPRESSOR */
	...
	/* ── Opus tuning ──────────────────────────────────────────────── */
	voxsdk_opus_config_t opus; /* all fields zero/false = use opus defaults */

	/* ── Jitter buffer ────────────────────────────────────────────── */
	voxsdk_jbuf_type_t jbuf_type;          /* adaptive (default) or fixed */
	 ...
	 * two fields below are reported by voxsdk_config_get() for
	...
	/* ── Tracing ─────────────────────────────────────────────────── */
	bool        trace_sip;      /* emit VOXSDK_EV_SIP_TRACE per message */
	bool        trace_sdp_diff; /* emit VOXSDK_EV_SDP_NEGOTIATION */
	const char *pcap_path;      /* NULL = no pcap; path = live capture */

	/* ── Logging & events ────────────────────────────────────────── */
	int                 log_level;      /* 0=err, 1=warn, 2=info, 3=debug */
	voxsdk_event_cb_t  event_cb;       /* required */
	void               *event_userdata;
	 ...
	 * by calling voxsdk_network_changed(); the built-in poller below is
	 ...
	 * build can be selected even without a voxsdk_codec_t constant.
	 * Same accepted spellings as the per-account
	 * voxsdk_account_config_t.audio_codec_names: "opus", "ulaw"/"pcmu",
	 * "alaw"/"pcma".
	 *
...
typedef struct {
	...
	voxsdk_media_enc_t  media_enc;   /* VOXSDK_MEDIA_ENC_NONE / SDES / DTLS_SRTP */
	...
	/* ── DTMF ──────────────────────────────────────────────────────────── */
	voxsdk_dtmf_mode_t dtmf_mode; /* default VOXSDK_DTMF_RFC4733 (0) */
...
VOXSDK_EXPORT void voxsdk_config_init(voxsdk_config_t *cfg);
 ...
 */
VOXSDK_EXPORT int voxsdk_init(const voxsdk_config_t *cfg);
 ...
 */
VOXSDK_EXPORT bool voxsdk_is_initialized(void);
 ...
 */
VOXSDK_EXPORT int voxsdk_set_event_handler(voxsdk_event_cb_t cb,
                                             void *userdata,
                                             bool deliver_owned_events);
 ...
 */
VOXSDK_EXPORT void voxsdk_shutdown(void);
 ...
 */
VOXSDK_EXPORT int voxsdk_account_create(const voxsdk_account_config_t *cfg,
                             voxsdk_account_handle_t *out);

/** Destroy account and all associated calls. Unregisters first if registered. */
VOXSDK_EXPORT void voxsdk_account_destroy(voxsdk_account_handle_t acct);
 ...
 *  VOXSDK_REG_FAILED. */
VOXSDK_EXPORT int voxsdk_account_register(voxsdk_account_handle_t acct);

/** Unregister (sends REGISTER with Expires: 0). */
VOXSDK_EXPORT int voxsdk_account_unregister(voxsdk_account_handle_t acct);
 ...
 */
VOXSDK_EXPORT int voxsdk_account_set_retry_policy(voxsdk_account_handle_t acct,
                                                     uint32_t initial_ms,
                                                     uint32_t max_ms,
                                                     float    backoff,
                                                     uint32_t max_attempts);
...
VOXSDK_EXPORT int voxsdk_account_cancel_retry(voxsdk_account_handle_t acct);
...
VOXSDK_EXPORT int voxsdk_account_retry_now(voxsdk_account_handle_t acct);
 ...
 */
VOXSDK_EXPORT int voxsdk_account_set_push_token(voxsdk_account_handle_t acct,
                                                   const char *push_token);
 ...
 */
VOXSDK_EXPORT int voxsdk_account_add_register_header(
 ...
 */
VOXSDK_EXPORT int voxsdk_account_add_header(voxsdk_account_handle_t acct,
                                const char *name, const char *value);
 ...
 */
VOXSDK_EXPORT int voxsdk_account_subscribe_presence(voxsdk_account_handle_t acct,
                                        const char *target_uri);
 ...
 * @return VOXSDK_OK on success, or negative error code
 */
VOXSDK_EXPORT int voxsdk_account_unsubscribe_presence(voxsdk_account_handle_t acct,
                                          const char *target_uri);
 ...
 */
VOXSDK_EXPORT void voxsdk_account_foreach(voxsdk_account_iter_fn fn, void *arg);
 ...
 */
VOXSDK_EXPORT int voxsdk_account_get_aor(voxsdk_account_handle_t acct,
                                            char *buf, size_t sz);
...
VOXSDK_EXPORT voxsdk_reg_state_t voxsdk_account_get_reg_state(
        voxsdk_account_handle_t acct);
 ...
 */
VOXSDK_EXPORT int voxsdk_call_invite(voxsdk_account_handle_t acct,
                         const char *uri,
                         voxsdk_call_handle_t *out);

/** Answer an incoming call (received via VOXSDK_EV_INCOMING_CALL). */
VOXSDK_EXPORT int voxsdk_call_answer(voxsdk_call_handle_t call);

/** Terminate a call with BYE. */
VOXSDK_EXPORT int voxsdk_call_hangup(voxsdk_call_handle_t call);
 ...
 */
VOXSDK_EXPORT int voxsdk_call_reject(voxsdk_call_handle_t call,
                                        uint16_t scode, const char *reason);

/** Put call on hold (re-INVITE with sendonly). */
VOXSDK_EXPORT int voxsdk_call_hold(voxsdk_call_handle_t call);

/** Resume a held call (re-INVITE with sendrecv). */
VOXSDK_EXPORT int voxsdk_call_resume(voxsdk_call_handle_t call);

/** Return true if the call is currently on local hold. */
VOXSDK_EXPORT bool voxsdk_call_is_held(voxsdk_call_handle_t call);

/** Send DTMF digit via RFC 4733 RTP events. digit: '0'-'9', '*', '#', 'A'-'D'. */
VOXSDK_EXPORT int voxsdk_call_send_dtmf(voxsdk_call_handle_t call, char digit);

/** Blind transfer via REFER. */
VOXSDK_EXPORT int voxsdk_call_transfer(voxsdk_call_handle_t call, const char *uri);
 ...
 * @param value  Header field value (e.g. "12345")
 */
VOXSDK_EXPORT int voxsdk_call_add_header(voxsdk_call_handle_t call,
                             const char *name, const char *value);
 ...
 */
VOXSDK_EXPORT int voxsdk_call_transfer_accept(voxsdk_call_handle_t  call,
                                                 voxsdk_call_handle_t *out);
 ...
 */
VOXSDK_EXPORT int voxsdk_call_transfer_reject(voxsdk_call_handle_t call,
                                                 uint16_t scode,
                                                 const char *reason);

VOXSDK_EXPORT int voxsdk_call_attended_transfer(voxsdk_call_handle_t call_a,
...
VOXSDK_EXPORT void voxsdk_call_foreach(voxsdk_call_iter_fn fn, void *arg);

/** The account this call belongs to, or NULL if it has none. */
VOXSDK_EXPORT voxsdk_account_handle_t voxsdk_call_get_account(
        voxsdk_call_handle_t call);

/** Current call state. Returns VOXSDK_CALL_ENDED for a NULL handle. */
VOXSDK_EXPORT voxsdk_call_state_t voxsdk_call_get_state(
        voxsdk_call_handle_t call);
 ...
 */
VOXSDK_EXPORT int voxsdk_message_send(voxsdk_account_handle_t account,
                          const char *to_uri,
                          const char *body,
 ...
 */
VOXSDK_EXPORT int voxsdk_account_publish_presence(voxsdk_account_handle_t account,
                                      voxsdk_presence_status_t status);
 ...
 */
VOXSDK_EXPORT int voxsdk_account_set_100rel(voxsdk_account_handle_t account,
 ...
 */
VOXSDK_EXPORT int voxsdk_audio_use_external(bool enable);
 ...
 */
VOXSDK_EXPORT int voxsdk_audio_external_push(const int16_t *pcm, size_t nsamp);
 ...
 */
VOXSDK_EXPORT int voxsdk_audio_external_pull(int16_t *pcm, size_t nsamp);
 ...
 */
VOXSDK_EXPORT int voxsdk_audio_external_format(uint32_t *srate, uint8_t *ch,
                                                  uint32_t *ptime);

/** True while a call is capturing or playing through the app-owned device. */
VOXSDK_EXPORT bool voxsdk_audio_external_is_active(void);
 ...
 */
VOXSDK_EXPORT int voxsdk_audio_list_input_devices(voxsdk_audio_device_t *devices,
                                                     int max_count);

/** Same for audio output (speaker/playback) devices. */
VOXSDK_EXPORT int voxsdk_audio_list_output_devices(voxsdk_audio_device_t *devices,
                                                      int max_count);
 ...
 */
VOXSDK_EXPORT void voxsdk_set_aec(bool enable);
 ...
 */
VOXSDK_EXPORT int voxsdk_set_aec_mode(voxsdk_aec_mode_t mode);
 ...
 */
VOXSDK_EXPORT void voxsdk_set_aec_suppression_level(float level);

/** Enable/disable noise suppression globally (takes effect next frame). */
VOXSDK_EXPORT void voxsdk_set_ns(bool enable);

/** Enable/disable automatic gain control globally (takes effect next frame). */
VOXSDK_EXPORT void voxsdk_set_agc(bool enable);
 ...
 * Takes effect on the next audio frame via atomic store; safe from any thread.
 */
VOXSDK_EXPORT void voxsdk_set_mic_gain_db(float db);
 ...
 * Takes effect on the next audio frame via atomic store; safe from any thread.
 */
VOXSDK_EXPORT void voxsdk_set_speaker_gain_db(float db);
 ...
 */
VOXSDK_EXPORT int voxsdk_call_set_dscp_rtp(voxsdk_call_handle_t call,
                                              uint8_t dscp);
 ...
 */
VOXSDK_EXPORT void voxsdk_set_jitter_buffer(uint32_t min_ms, uint32_t max_ms);

/** Set jitter buffer type (adaptive or fixed). Takes effect on new calls. */
VOXSDK_EXPORT void voxsdk_set_jitter_buffer_type(voxsdk_jbuf_type_t type);
 ...
 */
VOXSDK_EXPORT int voxsdk_call_set_rtp_timeout(voxsdk_call_handle_t call,
                                                 uint32_t seconds);
 ...
 * @return 0, or VOXSDK_ERR_INVAL / VOXSDK_ERR_STATE.
 */
VOXSDK_EXPORT int voxsdk_call_set_bitrate(voxsdk_call_handle_t call,
 ...
 */
VOXSDK_EXPORT void voxsdk_set_adaptive_bitrate(bool enabled,
                                                  uint32_t min_bps,
                                                  uint32_t max_bps);
 ...
 * @return 0, or VOXSDK_ERR_INVAL / VOXSDK_ERR_STATE.
 */
VOXSDK_EXPORT int voxsdk_account_keepalive_now(
                                        voxsdk_account_handle_t account);

/* ── Audio mute / device control ─────────────────────────────────────────── */

/** Mute/unmute the microphone (TX path) for a call. */
VOXSDK_EXPORT int voxsdk_audio_mute(voxsdk_call_handle_t call, bool mute);

/** Return true if TX audio is currently muted on this call. */
VOXSDK_EXPORT bool voxsdk_audio_is_muted(voxsdk_call_handle_t call);

/** Mute/unmute the speaker (RX path) for a call — silences incoming audio. */
VOXSDK_EXPORT int voxsdk_audio_mute_rx(voxsdk_call_handle_t call, bool mute);

/** Set the system audio input device by name. NULL = platform default. */
VOXSDK_EXPORT int voxsdk_audio_set_input_device(const char *name);

/** Set the system audio output device by name. NULL = platform default. */
VOXSDK_EXPORT int voxsdk_audio_set_output_device(const char *name);
 ...
 */
VOXSDK_EXPORT int voxsdk_call_set_media_tap(voxsdk_call_handle_t   call,
 ...
 */
VOXSDK_EXPORT int voxsdk_call_record_start(voxsdk_call_handle_t call,
                                              const char *path);

/** Stop recording and finalize WAV headers. Idempotent. */
VOXSDK_EXPORT int voxsdk_call_record_stop(voxsdk_call_handle_t call);
...
typedef struct {
	...
	voxsdk_transport_t  transport;  /* transport this dialog signals over */
	voxsdk_call_state_t state;      /* same value as the last CALL_STATE  */
} voxsdk_call_info_t;
 ...
 */
VOXSDK_EXPORT int voxsdk_call_get_info(voxsdk_call_handle_t  call,
 ...
 */
VOXSDK_EXPORT int voxsdk_call_get_stats(voxsdk_call_handle_t     call,
 ...
 */
VOXSDK_EXPORT int voxsdk_network_changed(void);

/**
 * Change the built-in interface poll interval at runtime.
 * @param seconds  Poll period; 0 disables polling entirely.
 */
VOXSDK_EXPORT int voxsdk_network_set_monitor_interval(uint32_t seconds);
 ...
 */
VOXSDK_EXPORT int voxsdk_network_set_handover_policy(bool reinvite_calls,
 ...
 */
VOXSDK_EXPORT int voxsdk_network_local_addr(char *buf, size_t sz);

/** False while the device has no usable (non-loopback) local address. */
VOXSDK_EXPORT bool voxsdk_network_is_up(void);
 ...
 */
VOXSDK_EXPORT int voxsdk_pcap_start(const char *path);

/** Stop capture and flush/close the pcap file. */
VOXSDK_EXPORT int voxsdk_pcap_stop(void);

#ifdef __cplusplus
}
#endif

#endif /* VOXSDK_H — embedded C declarations end */
...
using EventCallback = std::function<void(const voxsdk_event_t&)>;

class Call {
public:
    Call() = default;
    explicit Call(voxsdk_call_handle_t h) : h_(h) {}

    void answer()                        { detail::check(voxsdk_call_answer(h_),    "answer"); }
    void hangup()                        { voxsdk_call_hangup(h_); }
    void hold()                          { detail::check(voxsdk_call_hold(h_),      "hold"); }
    void resume()                        { detail::check(voxsdk_call_resume(h_),    "resume"); }
    void send_dtmf(char digit)           { voxsdk_call_send_dtmf(h_, digit); }
    void transfer(const std::string& u)  { detail::check(voxsdk_call_transfer(h_, u.c_str()), "transfer"); }
    void mute(bool m)                    { voxsdk_audio_mute(h_, m); }

    void add_header(const std::string& name, const std::string& value) {
        detail::check(voxsdk_call_add_header(h_, name.c_str(), value.c_str()), "add_header");
    }

    /* Incoming REFER (VOXSDK_EV_TRANSFER_REQUEST). Answer with exactly one
     * of these, or the transferor never learns the outcome. */
    Call transfer_accept() {
        voxsdk_call_handle_t nh = nullptr;
        detail::check(voxsdk_call_transfer_accept(h_, &nh), "transfer_accept");
        return Call(nh);
    }
    void transfer_reject(uint16_t scode = 603, const std::string& reason = "Declined") {
        detail::check(voxsdk_call_transfer_reject(h_, scode, reason.c_str()),
                      "transfer_reject");
    }

    voxsdk_call_info_t info() const {
        voxsdk_call_info_t i{};
        detail::check(voxsdk_call_get_info(h_, &i), "get_info");
        return i;
    }

    voxsdk_ev_media_stats_t stats() const {
        voxsdk_ev_media_stats_t s{};
        voxsdk_call_get_stats(h_, &s);
        return s;
    }

    void attended_transfer(Call& consult) {
        detail::check(voxsdk_call_attended_transfer(h_, consult.h_), "attended_transfer");
    }

    void set_media_tap(voxsdk_media_tap_cb_t cb, void* userdata = nullptr) {
        detail::check(voxsdk_call_set_media_tap(h_, cb, userdata), "set_media_tap");
    }

    voxsdk_call_handle_t handle() const { return h_; }
    explicit operator bool()       const { return h_ != nullptr; }

private:
    voxsdk_call_handle_t h_ = nullptr;
};
...
public:
    ...
    Account(voxsdk_account_handle_t h, EventCallback* global_cb)
        : h_(h), global_cb_(global_cb) {}

    ~Account() { if (h_) voxsdk_account_destroy(h_); }
    ...
    Account& operator=(Account&& o) noexcept {
        if (this != &o) {
            if (h_) voxsdk_account_destroy(h_);
            h_         = o.h_;
    ...
    void register_account() {
        detail::check(voxsdk_account_register(h_), "register_account");
    }

    void unregister() { voxsdk_account_unregister(h_); }

    Call call(const std::string& uri) {
        voxsdk_call_handle_t ch = nullptr;
        detail::check(voxsdk_call_invite(h_, uri.c_str(), &ch), "call_invite");
        return Call(ch);
    }
    ...
    void send_message(const std::string& to, const std::string& body,
        ...
        detail::check(
            voxsdk_message_send(h_, to.c_str(), body.c_str(), content_type.c_str()),
            "message_send");
    }

    void publish_presence(voxsdk_presence_status_t status) {
        voxsdk_account_publish_presence(h_, status);
    }

    void subscribe_presence(const std::string& target_uri) {
        detail::check(voxsdk_account_subscribe_presence(h_, target_uri.c_str()),
                      "subscribe_presence");
    }

    void unsubscribe_presence(const std::string& target_uri) {
        detail::check(voxsdk_account_unsubscribe_presence(h_, target_uri.c_str()),
                      "unsubscribe_presence");
    }

    void add_header(const std::string& name, const std::string& value) {
        detail::check(voxsdk_account_add_header(h_, name.c_str(), value.c_str()), "add_header");
    }
    ...
    void add_register_header(const std::string& name, const std::string& value) {
        detail::check(
            voxsdk_account_add_register_header(h_, name.c_str(), value.c_str()),
            "add_register_header");
    }

    /** Update the push token at runtime. Pass empty string to clear push params. */
    int set_push_token(const std::string& push_token) {
        return voxsdk_account_set_push_token(
            h_, push_token.empty() ? nullptr : push_token.c_str());
    }

    void set_100rel(voxsdk_100rel_mode_t mode) {
        detail::check(voxsdk_account_set_100rel(h_, mode), "set_100rel");
    }

    voxsdk_account_handle_t handle() const { return h_; }
    explicit operator bool()          const { return h_ != nullptr; }

private:
    voxsdk_account_handle_t h_      = nullptr;
    EventCallback*           global_cb_ = nullptr;
};

class SDK {
public:
    SDK() { voxsdk_config_init(&cfg_); }

    ~SDK() { if (initialized_) voxsdk_shutdown(); }

    SDK(const SDK&)            = delete;
    SDK& operator=(const SDK&) = delete;

    voxsdk_config_t& config() { return cfg_; }
    const voxsdk_config_t& config() const { return cfg_; }
    ...
    void init() {
        ...
        detail::check(voxsdk_init(&cfg_), "voxsdk_init");
        initialized_ = true;
    }

    Account create_account(const std::string& uri,
                           const std::string& password,
                           voxsdk_transport_t transport = VOXSDK_TRANSPORT_UDP,
                           std::initializer_list<voxsdk_codec_t> codecs = {}) {
        ...
        voxsdk_account_handle_t h = nullptr;
        detail::check(voxsdk_account_create(&acfg, &h), "account_create");
    ...
    Account create_account(const voxsdk_account_config_t& acfg) {
        if (!initialized_) init();
    ...
    void pcap_start(const std::string& path) {
        detail::check(voxsdk_pcap_start(path.c_str()), "pcap_start");
    }

    void pcap_stop() { voxsdk_pcap_stop(); }

    const char* version() const { return voxsdk_version(); }

    // ── Echo cancellation ────────────────────────────────────────────────
    /** Enable or disable echo cancellation. Re-enables the aec_mode set at init. */
    void set_aec(bool enable) { voxsdk_set_aec(enable); }

    /** Switch AEC backend at runtime. Only AEC_OFF ↔ init_mode transitions allowed. */
    int set_aec_mode(voxsdk_aec_mode_t mode) { return voxsdk_set_aec_mode(mode); }

    /** Tune suppressor aggressiveness: 0=none, 1=maximum (default). SUPPRESSOR only. */
    void set_aec_suppression_level(float level) { voxsdk_set_aec_suppression_level(level); }

    // ── Noise suppression / AGC ──────────────────────────────────────────
    void set_ns(bool enable)  { voxsdk_set_ns(enable); }
    void set_agc(bool enable) { voxsdk_set_agc(enable); }

    // ── Gain control ─────────────────────────────────────────────────────
    /** Set microphone (TX) gain in dB. Range: -20 to +20. 0 = unity. */
    void set_mic_gain(float db) { voxsdk_set_mic_gain_db(db); }

    /** Set speaker (RX) gain in dB. Range: -20 to +20. 0 = unity. */
    void set_speaker_gain(float db) { voxsdk_set_speaker_gain_db(db); }

    void set_jitter_buffer(uint32_t min_ms, uint32_t max_ms) {
        voxsdk_set_jitter_buffer(min_ms, max_ms);
    }
    ...
    void use_external_audio(bool enable) {
        detail::check(voxsdk_audio_use_external(enable), "use_external_audio");
    }
    ...
    int audio_push(const int16_t* pcm, size_t nsamp) {
        return voxsdk_audio_external_push(pcm, nsamp);
    }
    ...
    int audio_pull(int16_t* pcm, size_t nsamp) {
        return voxsdk_audio_external_pull(pcm, nsamp);
    }
    ...
    bool audio_format(AudioFormat& out) {
        return 0 == voxsdk_audio_external_format(&out.srate, &out.ch,
                                                  &out.ptime);
    }
    ...
    bool audio_external_active() {
        return voxsdk_audio_external_is_active();
    }

    /* ── Network handover (Wi-Fi ↔ 4G/5G) ──────────────────────────────── */

    /** Tell the SDK the network may have changed.  Progress arrives as
     *  VOXSDK_EV_NETWORK events.  Safe to call from any thread. */
    void network_changed() {
        detail::check(voxsdk_network_changed(), "network_changed");
    }
    ...
    void network_set_monitor_interval(uint32_t seconds) {
        detail::check(voxsdk_network_set_monitor_interval(seconds),
                      "network_set_monitor_interval");
    }

    void network_set_handover_policy(bool reinvite_calls,
                                     bool hangup_on_failure) {
        detail::check(voxsdk_network_set_handover_policy(reinvite_calls,
                                                          hangup_on_failure),
    ...
    std::string network_local_addr() {
        char buf[64] = {0};
        if (voxsdk_network_local_addr(buf, sizeof(buf)) != VOXSDK_OK)
            return {};
        return buf;
    }

    bool network_is_up() { return voxsdk_network_is_up(); }

private:
    static void dispatch_(const voxsdk_event_t* ev, void* ud) {
        auto* self = static_cast<SDK*>(ud);
        if (self->event_cb_) self->event_cb_(*ev);
    }

    voxsdk_config_t cfg_{};
