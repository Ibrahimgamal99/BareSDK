...
int voxsdk_call_record_start(voxsdk_call_handle_t call, const char *path)
{
	if (!call || !path) return VOXSDK_ERR_INVAL;

	rec_start_ctx_t ctx = {.lc = call, .result = 0};
	str_ncpy(ctx.path, path, sizeof(ctx.path));

	return vox_dispatch_sync(record_start_fn, &ctx);
}
...
int voxsdk_call_record_stop(voxsdk_call_handle_t call)
{
	...
	return vox_dispatch_sync(record_stop_fn, call);
}
