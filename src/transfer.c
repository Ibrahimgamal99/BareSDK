 ...
 *
 * Blind transfer   — voxsdk_call_transfer() (in call.c)
 *                    sends REFER with Refer-To: <uri>
 *
 * Attended transfer — voxsdk_call_attended_transfer(call_a, call_b)
 ...
 *                    Surfaced as VOXSDK_EV_TRANSFER_REQUEST.
 */
...
typedef struct {
	struct voxsdk_call *call_a;
	struct voxsdk_call *call_b;
	int                  result;
} atxfer_ctx_t;
...
{
	if (!call_a || !call_b) return VOXSDK_ERR_INVAL;
	atxfer_ctx_t ctx = {.call_a = call_a, .call_b = call_b, .result = 0};
	int err = vox_dispatch_sync(attended_transfer_fn, &ctx);
...
void vox_transfer_handle_event(struct bevent *event)
{
	...
	struct voxsdk_call    *lc   = bc ? vox_call_find(bc)          : NULL;
	...
	qev->ev.type = VOXSDK_EV_TRANSFER_REQUEST;
	voxsdk_ev_transfer_req_t *tr = &qev->ev.u.transfer_req;
	tr->call    = lc;
	tr->account = acct;
	 ...
	 * us for the terminating NOTIFY — see voxsdk_call_transfer_accept(). */
...
static void transfer_accept_fn(void *arg)
{
	...
	struct voxsdk_call *new_lc = NULL;
	...
	if (err) {
		warning("VoxSDK/transfer: ua_call_alloc failed (%m)\n", err);
	...
	new_lc = vox_call_wrap_new(call2, lc->acct, VOXSDK_CALL_CALLING, true);
	if (!new_lc) {
	...
	if (err) {
		warning("VoxSDK/transfer: connect to '%s' failed (%m)\n",
	...
	vox_call_setup_watch_start(new_lc);
                                 ...
                                 voxsdk_call_handle_t *out)
{
	...
	int err = vox_dispatch_sync(transfer_accept_fn, &ctx);
	return err ? err : ctx.result;
}
                                 ...
                                 uint16_t scode, const char *reason)
{
	...
	int err = vox_dispatch_sync(transfer_reject_fn, &ctx);
	return err ? err : ctx.result;
}
