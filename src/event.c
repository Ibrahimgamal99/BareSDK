/**
 * @file event.c  baresip bevent → voxsdk_event_t bridge
 *
 * baresip's global event handler fires on re_main. We CANNOT call consumer
 * callbacks from re_main because consumers may call back into VoxSDK
 * (e.g. voxsdk_call_answer() from inside VOXSDK_EV_INCOMING_CALL), which
 ...
 * (deref_after_deliver — see the field comment in voxsdk_internal.h).  That
 * reference has to be released on *every* path the event can die on: normal
 * delivery, voxsdk_event_release() in owned mode, and the queue-full drop
 * inside vox_event_post_qev().
 *
...
{
	struct voxsdk_queued_event *qev = arg;
 ...
 * this file. */
struct voxsdk_queued_event *vox_qev_alloc(void)
{
	return mem_zalloc(sizeof(struct voxsdk_queued_event), qev_destructor);
}
...
static const char *rb(const char *p,
                      const struct voxsdk_queued_event *o,
                      const struct voxsdk_queued_event *n)
{
 ...
 * Keep the switch in sync with the voxsdk_event_t union in voxsdk.h
...
static struct voxsdk_queued_event *
qev_clone(struct voxsdk_queued_event *old)
{
	struct voxsdk_queued_event *n = vox_qev_alloc();
	if (!n)
		return NULL;
	...
	case VOXSDK_EV_MEDIA_STATS:
		...
		break;
	case VOXSDK_EV_REGISTRAR_WARNING:
	...
	case VOXSDK_EV_TRANSFER_REQUEST:
		...
		break;
	case VOXSDK_EV_TRANSFER_FAILED:
...
void voxsdk_event_release(const voxsdk_event_t *ev)
{
	...
	struct voxsdk_queued_event *qev = (struct voxsdk_queued_event *)
		((char *)(uintptr_t)ev -
		 offsetof(struct voxsdk_queued_event, ev));
...
static int event_thread_fn(void *arg)
{
	...
	for (;;) {
		...
		while (list_isempty(&g_vox.ev_queue) && !g_vox.ev_shutdown)
			cnd_wait(&g_vox.ev_cond, &g_vox.ev_lock);

		/* Drain the whole queue before checking shutdown again */
		struct le *le = list_head(&g_vox.ev_queue);
		if (le) {
			struct voxsdk_queued_event *qev = le->data;
			list_unlink(&qev->le);
			g_vox.ev_queue_len--;
			/* Snapshot the handler while still holding ev_lock:
			 * voxsdk_set_event_handler() may swap all three fields
			...
			voxsdk_event_cb_t  cb    = g_vox.cfg.event_cb;
			void               *cb_ud = g_vox.cfg.event_userdata;
			bool                owned = g_vox.cfg.deliver_owned_events;
			...
			g_vox.ev_delivering = true;
			mtx_unlock(&g_vox.ev_lock);

			if (cb) {
				if (owned) {
					/* Hand the consumer an owned clone; it
					 * frees it via voxsdk_event_release().
					 * OOM → event dropped. */
					struct voxsdk_queued_event *own =
						qev_clone(qev);
					if (own)
						cb(&own->ev, cb_ud);
				}
			...
			cnd_broadcast(&g_vox.ev_idle_cond);
		} else if (g_vox.ev_shutdown) {
			break;
		}
	}
...
bool vox_event_post_qev(struct voxsdk_queued_event *qev)
{
	mtx_lock(&g_vox.ev_lock);
	if (g_vox.ev_queue_len >= g_vox.ev_queue_max) {
		...
		mtx_unlock(&g_vox.ev_lock);
		mem_deref(qev);
		warning("VoxSDK/event: dropped event type %d (queue full)\n", type);
		return false;
	}
...
void vox_event_post(const voxsdk_event_t *ev)
{
	struct voxsdk_queued_event *qev = vox_qev_alloc();
	if (!qev) {
		warning("VoxSDK/event: dropped event type %d (OOM)\n", ev->type);
		return;
	}
	memcpy(&qev->ev, ev, sizeof(*ev));
	vox_event_post_qev(qev);  /* warns and frees qev on failure */
}
...
static bool codec_reason_is_really_menc(const char *reason,
                                        const struct voxsdk_account *acct)
{
	...
	enc = acct->cfg.media_enc ? acct->cfg.media_enc : g_vox.cfg.media_enc;
	return enc == VOXSDK_MEDIA_ENC_NONE;
}
                            ...
                            void *arg)
{
	...
	struct voxsdk_account *acct =
		ua ? vox_account_find_by_ua(ua) : NULL;
	struct voxsdk_call    *lc   =
		bc ? vox_call_find(bc) : NULL;
	...
	case BEVENT_REGISTERING:
		...
		if (acct && acct->reconnecting) {
			...
			ev.u.reg.state = VOXSDK_REG_REGISTERING;
			if (acct)
	...
	case BEVENT_FALLBACK_FAIL: {   /* see FALLBACK_OK above */
		...
		if (code == 401 || code == 403 || code == 407)
			ev.u.reg.error = VOXSDK_ERR_AUTH;
		else if (code >= 500 && code < 600)
			ev.u.reg.error = VOXSDK_ERR_SERVER_5XX;
		else
			ev.u.reg.error = VOXSDK_ERR_TRANSPORT;
		...
		ev.u.reg.state = acct
		        ? vox_account_reg_fail_state(acct, ev.u.reg.error)
		        : VOXSDK_REG_FAILED;
		...
		if (acct) {
			 ...
			 * recovery. */
			if (ev.u.reg.error != VOXSDK_ERR_AUTH)
	...
	case BEVENT_UNREGISTERING:
		ev.type        = VOXSDK_EV_REG_STATE;
		ev.u.reg.state = VOXSDK_REG_UNREGISTERING;
		ev.u.reg.account = acct;
		if (acct) {
	...
	case BEVENT_CALL_INCOMING: {
		...
		struct voxsdk_call *new_lc =
			vox_call_wrap_new(bc, acct, VOXSDK_CALL_RINGING, false);
		if (!new_lc) { post = false; break; }

		struct voxsdk_queued_event *qev = vox_qev_alloc();
		if (!qev) {
			vox_call_unregister(new_lc);
			mem_deref(new_lc);
			post = false;
			break;
		}
		qev->ev.type = VOXSDK_EV_INCOMING_CALL;
	...
	case BEVENT_CALL_PROGRESS:   /* 183 w/ SDP — alerting, early media may
		...
		ev.type = VOXSDK_EV_CALL_STATE;
		ev.u.call_state.call    = lc;
		ev.u.call_state.account = acct;
		ev.u.call_state.state   = VOXSDK_CALL_RINGING;
		if (lc) lc->state = VOXSDK_CALL_RINGING;
		break;
	...
	case BEVENT_CALL_ESTABLISHED:
		...
		ev.u.call_state.call    = lc;
		ev.u.call_state.account = acct;
		ev.u.call_state.state   = VOXSDK_CALL_ESTABLISHED;
		if (lc) {
			lc->state            = VOXSDK_CALL_ESTABLISHED;
			lc->stats_call_start = tmr_jiffies();
			/* The call is up — the setup watchdog has nothing left to
			 * guard against. */
			vox_call_setup_watch_cancel(lc);
	...
	case BEVENT_CALL_CLOSED: {
		 ...
		 * transport itself is covered by REG_STATE and VOXSDK_EV_NETWORK, which
		...
		if (scode == 487)
			...
			ev.u.call_state.state = VOXSDK_CALL_ENDED;
		...
		if (scode == 401 || scode == 403 || scode == 407)
			ev.u.call_state.error = VOXSDK_ERR_AUTH;
		/* 408 is what the setup watchdog cancels with, and what a proxy
		 * sends when its own transaction timed out — both are timeouts,
		 * not the malformed-request sense of VOXSDK_ERR_INVAL. */
		else if (scode == 408)
			ev.u.call_state.error = VOXSDK_ERR_TIMEOUT;
		else if (scode >= 500 && scode < 600)
			ev.u.call_state.error = VOXSDK_ERR_SERVER_5XX;
		else if (scode >= 400)
			ev.u.call_state.error = VOXSDK_ERR_INVAL;
		else if (is_transport_err && !peer_hangup)
			ev.u.call_state.error = VOXSDK_ERR_TRANSPORT;

		if (lc) {
			vox_stats_collect_final(lc);  /* must be before bc = NULL */
			lc->state = ev.u.call_state.state;
			lc->bc    = NULL; /* baresip will free the call */
			vox_call_unregister(lc);
		}
	...
	case BEVENT_CALL_HOLD:
		...
		ev.u.call_state.call    = lc;
		ev.u.call_state.account = acct;
		ev.u.call_state.state   = VOXSDK_CALL_HELD;
		if (lc) lc->state = VOXSDK_CALL_HELD;
		break;
	...
	case BEVENT_CALL_REMOTE_SDP: {
		...
		vox_sdp_handle_event(bev, event);
	...
	case BEVENT_CALL_TRANSFER_FAILED: {
		...
		const char *reason = bevent_get_text(event);
		ev.type = VOXSDK_EV_TRANSFER_FAILED;
	 ...
	 *
	 *   OUTGOING  fires inside ua_connect(), before voxsdk_call_invite()
	 ...
	 *             its own timer for VOXSDK_EV_MEDIA_STATS.
	 ...
	 *             voxsdk_account_create() returned it the handle.
	 */
	case BEVENT_CREATE:
	...
	default:
		 ...
		 * app could not tell. */
		debug("VoxSDK: unhandled baresip event %s (%d)\n",
...
int vox_event_init(void)
{
	g_vox.ev_shutdown = false;

	int rc = thrd_create(&g_vox.ev_thread, event_thread_fn, NULL);
	if (rc != thrd_success)
		return ENOMEM;

	g_vox.ev_thread_started = true;

	return bevent_register(bevent_handler, NULL);
}

void vox_event_close(void)
{
	 ...
	 *
	 * Guarded because this also runs from voxsdk_init()'s failure path,
	...
	if (g_vox.ev_thread_started) {
		mtx_lock(&g_vox.ev_lock);
		g_vox.ev_shutdown = true;
		cnd_signal(&g_vox.ev_cond);
		mtx_unlock(&g_vox.ev_lock);

		thrd_join(g_vox.ev_thread, NULL);
		g_vox.ev_thread_started = false;
	}

	/* Free any remaining queued events */
	struct le *le, *le_tmp;
	LIST_FOREACH_SAFE(&g_vox.ev_queue, le, le_tmp) {
		struct voxsdk_queued_event *qev = le->data;
		list_unlink(&qev->le);
		mem_deref(qev);
	}

	/* Keep the counter with the list.  voxsdk_init() re-zeroes it, so this
