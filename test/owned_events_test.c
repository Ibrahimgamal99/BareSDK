...
static const baresdk_event_t *g_stash[MAX_EVENTS];
...
static void event_cb(const baresdk_event_t *ev, void *ud)
{
	...
	if (!g_owned_mode) {
		...
		if (ev->type == BARESDK_EV_LOG && ev->u.log.message)
...
static void *consumer_fn(void *arg)
{
	...
	for (;;) {
		...
		const baresdk_event_t *ev = g_stash[tail];
		switch (ev->type) {
		case BARESDK_EV_LOG:
...
int main(int argc, char **argv)
{
	...
	if (err) {
		fprintf(stderr, "FAIL: baresdk_init: %d\n", err);
		return 1;
	}
	...
	baresdk_account_set_retry_policy(acct, 200, 1000, 1.5f, 0);
