 ...
 * voxsdk_event_release() — even when released from a different thread.
 *
...
static const voxsdk_event_t *g_stash[MAX_EVENTS];
...
static void event_cb(const voxsdk_event_t *ev, void *ud)
{
	(void)ud;
	if (!g_owned_mode) {
		/* borrowed mode: touch strings inline only (legacy contract) */
		if (ev->type == VOXSDK_EV_LOG && ev->u.log.message)
...
static void *consumer_fn(void *arg)
{
	...
	for (;;) {
		...
		const voxsdk_event_t *ev = g_stash[tail];
		switch (ev->type) {
		case VOXSDK_EV_LOG:
...
int main(int argc, char **argv)
{
	...
	if (err) {
		fprintf(stderr, "FAIL: voxsdk_init: %d\n", err);
		return 1;
	}
	...
	voxsdk_account_set_retry_policy(acct, 200, 1000, 1.5f, 0);
