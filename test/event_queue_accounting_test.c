 ...
 * baresdk_network_local_addr() instead, which is the address it did bind.
...
int main(void)
{
	...
	rc = baresdk_init(&cfg);
	CHECK(rc == BARESDK_OK, "baresdk_init returned %d (%s)\n",
	      rc, baresdk_strerror(rc));
	...
	rc = baresdk_account_create(&acfg, &acct);
	CHECK(rc == BARESDK_OK, "baresdk_account_create returned %d (%s)\n",
	      rc, baresdk_strerror(rc));

	rc = baresdk_account_register(acct);
	CHECK(rc == BARESDK_OK, "baresdk_account_register returned %d (%s)\n",
	      rc, baresdk_strerror(rc));

	/* Let the stack finish binding its transports before posting to them. */
	sleep(1);

	CHECK(baresdk_network_local_addr(laddr, sizeof(laddr)) == BARESDK_OK &&
	      laddr[0],
	      "baresdk_network_local_addr() gave no address — this host has no "
	...
	CHECK(atomic_load(&g_msg_seen),
	      "no BARESDK_EV_MESSAGE arrived — the enqueue path under test was "
