 ...
 *
 * Lets Android code drive voxsdk_audio_external_push()/pull() directly from a
 ...
 *
 * The entry points live in libvoxsdk.so rather than in a separate shim so
 ...
 *
 * The Java class is deliberately `dev.voxsdk.ExternalAudio`, not anything
 * under `dev.voxsdk.flutter`: this is a plain Android binding, and a Flutter
...
#define JNI_FN(name) JNICALL Java_dev_voxsdk_ExternalAudio_##name
...
{
	(void)env; (void)cls;
	return voxsdk_audio_use_external(enable == JNI_TRUE);
}
                                  ...
                                  jint nsamp)
{
	...
	return voxsdk_audio_external_push(pcm, (size_t)nsamp);
}
                                  ...
                                  jint nsamp)
{
	...
	return voxsdk_audio_external_pull(pcm, (size_t)nsamp);
}
...
{
	(void)env; (void)cls;
	return voxsdk_audio_external_is_active() ? JNI_TRUE : JNI_FALSE;
}
