...
The `VoxSDK` Flutter package (`bindings/flutter/`) is a hybrid plugin:

- **Dart FFI** drives the native SIP/media core (`libvoxsdk`).
- On **Android** the plugin ships prebuilt `libvoxsdk.so` for
  `arm64-v8a`, `armeabi-v7a` and `x86_64` inside its `jniLibs`, plus a small
  Kotlin shim (`VoxSDKPlugin`) that provides the app cache dir, voice-call
  audio focus, speakerphone routing, and network-change callbacks.
- On **iOS** the plugin vendors a prebuilt dynamic `VoxSDK.xcframework`
  (device arm64 + simulator arm64/x86_64) plus a Swift shim
  (`VoxSDKPlugin`) for audio-session activation, speakerphone routing, and
...
- **Desktop** (Linux/Windows/macOS) works as a plain FFI binding — the app
  bundles the native library itself (or passes `VoxSDK(libPath: ...)`).
...
dependencies:
  vox_sdk:
    git:
      url: https://github.com/Ibrahimgamal99/VoxSDK.git
      path: bindings/flutter
      ref: <full-40-char-sha>   # pin for anything you ship
  # or, from a checkout:
  # vox_sdk:
  #   path: path/to/VoxSDK/bindings/flutter
```
...
unzip -p build/app/outputs/flutter-apk/app-release.apk \
  lib/arm64-v8a/libvoxsdk.so | md5sum
md5sum bindings/flutter/android/src/main/jniLibs/arm64-v8a/libvoxsdk.so
```
...
`bindings/flutter/ios/Frameworks/VoxSDK.xcframework` — it is built by CI
...
`VoxSDK.start()` is required on Android: it injects the app cache dir as
...
`VoxSDK` from an isolate that already has one throws until you call
`shutdown()`.
...
`VoxSDK.start()` reattaches to it instead of failing:

```dart
final sdk = await VoxSDK.start(config: cfg);
...
(`VoxSDKConfig.regRetry*` or per-account `setRetryPolicy`). On a network
...
supported API level). Opus can be tuned via `VoxSDKConfig.opus`
...
```dart
final sdk = await VoxSDK.start(
  config: const VoxSDKConfig(appOwnedAudio: true),
);
...
[`AppOwnedAudioEngine.kt`](../../bindings/flutter/android/src/main/kotlin/dev/voxsdk/flutter/AppOwnedAudioEngine.kt)
(`AudioRecord`/`AudioTrack` on `VOICE_COMMUNICATION`) and
[`VoxSDKExternalAudio.m`](../../bindings/flutter/ios/Classes/VoxSDKExternalAudio.m)
(a `VoiceProcessingIO` AudioUnit).

**The realtime loop is native on purpose.** `push`/`pull` are reachable from
Dart FFI but deliberately absent from the `VoxSDK` API: they run on a
...
`voxsdk_audio_external_push/pull/format` — from your own audio thread. On
Android the JNI entry points are already exported from `libvoxsdk.so` as
`dev.voxsdk.ExternalAudio`, so no NDK build is needed:
...
`VoxSDKConfig.traceSip`), `SdpNegotiationEvent` (enable `traceSdpDiff`),
...
VOXSDK_BUILD_ROOT=$HOME/.cache/voxsdk-build \
...
# Regenerate FFI bindings after changing include/voxsdk.h:
cd bindings/flutter && dart run ffigen --config ffigen.yaml
```

`VOXSDK_BUILD_ROOT` should point at a native filesystem when the repo
...
`bindings/flutter/ios/Frameworks/VoxSDK.xcframework`) — there is no second
...
`bindings/flutter/test/vox_sdk_smoke_test.dart` (needs
