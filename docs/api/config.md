...
Always start with `voxsdk_config_init(&cfg)` to zero-fill and set `version`/`struct_size`.
...
|---|---|---|---|
| `version` | `uint32_t` | 1 | Set by `voxsdk_config_init`. Do not change. |
| `struct_size` | `size_t` | `sizeof(voxsdk_config_t)` | Set by `voxsdk_config_init`. Do not change. |

### Transport

| Field | Type | Default | Description |
|---|---|---|---|
| `transport` | `voxsdk_transport_t` | `UDP` | Default transport: UDP, TCP, TLS, WS, WSS |
...
wire, no event, and `voxsdk_call_invite()` already returned success.
...
| Field | Type | Default | Description |
|---|---|---|---|
| `media_enc` | `voxsdk_media_enc_t` | `NONE` | `NONE`, `SDES`, `DTLS_SRTP` |
...
is what fixes it.
| `audio_codecs[8]` | `voxsdk_codec_t[]` | `[OPUS]` | Preference-ordered codec list |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `aec_mode` | `voxsdk_aec_mode_t` | `SUPPRESSOR` | `OFF`, `SUPPRESSOR`, `WEBRTC` |
...
runtime switch (`voxsdk_audio_use_external()`), so it can be flipped mid-call
and does not survive a restart. Flutter exposes `VoxSDKConfig.appOwnedAudio`
as convenience only — it is applied straight after `init()`, not marshalled into
`voxsdk_config_t`. See [App-owned audio device](media.md#app-owned-audio-device).
...
| `jbuf_type` | `voxsdk_jbuf_type_t` | `ADAPTIVE` | `ADAPTIVE` or `FIXED` depth |
...
| `sip_timer_b_ms` | `uint32_t` | 32000 | An outgoing call still in `CALLING` after this long is cancelled with 408 → `CALL_FAILED` / `VOXSDK_ERR_TIMEOUT`. 8000–12000 to fail fast on mobile; 0 disables. Only `CALLING` is watched — a call that reached `RINGING` has proven the path works |
| `sip_timer_f_ms` | `uint32_t` | 32000 | A REGISTER with no answer after this long reports `VOXSDK_ERR_TIMEOUT` and hands over to the retry policy; 0 disables |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `stats_interval_ms` | `uint32_t` | 2000 | Poll period for `VOXSDK_EV_MEDIA_STATS`; 0 disables. Also the master switch for RTCP accounting, so with 0 the loss/jitter/RTT/MOS fields read back as zero everywhere and quality alerts, media-stall detection and adaptive bitrate are all inert. Python: `call.poll_stats(interval=)` overrides this per-call |
| `mos_method` | `voxsdk_mos_method_t` | `EMODEL` | `EMODEL` or `SIMPLIFIED` |
...
| `trace_sip` | `bool` | false | Emit `VOXSDK_EV_SIP_TRACE` per message |
| `trace_sdp_diff` | `bool` | false | Emit `VOXSDK_EV_SDP_NEGOTIATION` |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `net_monitor_interval_s` | `uint32_t` | 10 | Interface poll period in seconds; 0 = off. Set 0 on mobile and call `voxsdk_network_changed()` from the OS callback |
...
| `net_ice_handover` | `voxsdk_ice_handover_t` | `BEST_EFFORT` | Applies only to an ICE call that could **not** be re-gathered (ICE calls are normally migrated with a full ICE restart): `BEST_EFFORT` runs the full retry budget, `FAIL_FAST` gives up after one attempt — see [Network handover](../guides/network_handover.md#ice-calls) |
...
| `event_cb` | `voxsdk_event_cb_t` | required | Your event callback |
...
you **must** call `voxsdk_event_release(ev)` exactly once per delivered event —
...
`voxsdk_set_event_handler(cb, userdata, deliver_owned_events)` re-points event
...
| `transport` | `voxsdk_transport_t` | No | Overrides global default |
...
| `media_enc` | `voxsdk_media_enc_t` | No | Per-account media encryption override |
...
| `push_provider` | `voxsdk_push_provider_t` | No | `NONE`, `APNS`, `FCM` — see [Accounts → Push notifications](accounts.md#push-notifications) |
...
| `audio_codecs[8]` | `voxsdk_codec_t[]` | No | Per-account preference-ordered codec list |
...
| `dtmf_mode` | `voxsdk_dtmf_mode_t` | No | `RFC4733` (default), `SIP_INFO`, `AUTO` |
