...
All notable changes to VoxSDK are documented here.
...
- **Call quality was scored from lifetime numbers, so one early burst of loss
  ...
  is non-linear in R. `voxsdk_call_get_stats()` now zeroes `out` before
...
- **Documentation: the event-callback contract was documented backwards.**
  `voxsdk_event_cb_t` said "do not call VoxSDK APIs from inside this
  ...
  inside the handler. The comment in `include/voxsdk.h`, the C++ wrapper and
...
- **The BYE and in-dialog WS routing fixes now hold on every platform — and
  ...
  renamed to `__real_*` behind compile-time switches so VoxSDK's `ws_path.c`
...
- **One incoming SIP MESSAGE, presence NOTIFY, MWI NOTIFY or REFER used to
  ...
  producer refuses to enqueue once the length reaches `VOXSDK_EV_QUEUE_MAX`.
  ...
  All four now go through `vox_event_post_qev()`, which has always done the
...
- **A queued event that was dropped leaked the call it described.** The
  queue-full path in `vox_event_post_qev()` freed the event with a bare
  ...
  call site cannot forget. All 17 allocation sites go through `vox_qev_alloc()`.
...
- **Any failure inside `voxsdk_init()` crashed the host application.** The
  unwind path called `vox_event_close()` unconditionally, which joined an event
...
- **A network handover that ran out of attempts stranded every live call.**
  ...
  and the app was never told. They now get `VOXSDK_NET_CALL_MIGRATION_FAILED`
...
- **A stale ICE-restart flag disabled ICE restart for the rest of a call.** When
  ...
  later `vox_ice_restart()` returned `EALREADY`, so a second network handover
...
- **A network change re-REGISTERed accounts whose credentials the registrar had
  ...
  `VOXSDK_ERR_AUTH` are now left alone. Every other failure — transport,
  timeout, 5xx — is still retried, because a new network is exactly what might
  fix those.

- **Python wheels published from CI had a corrupt `voxsdk_config_t` layout.**
  The release workflow generated the cffi header with its own copy of the
  `gcc -E` pipeline, which was missing `-DVOXSDK_NO_PACKED_ENUM=1` while still
  stripping `__attribute__`. cffi therefore saw `voxsdk_aec_mode_t` as 4 bytes
  ...
  `version`/`struct_size` guard could not catch it, because `voxsdk_config_init()`
...
- **`voxsdk_call_transfer_accept()` / `voxsdk_call_transfer_reject()` — an
  incoming REFER can finally be answered.** The SDK raised
  `VOXSDK_EV_TRANSFER_REQUEST` and then had nothing to offer: no way to follow
  ...
  `voxsdk_call_transfer_accept()` places the call *linked* to the original, so
  the stack reports the outcome by itself — `200 OK` once the new call is
  established, or the failure status if it never is.
  `voxsdk_call_transfer_reject(call, scode, reason)` sends the terminating
  NOTIFY and leaves the call up, so the user stays on the line. Answer every
  `VOXSDK_EV_TRANSFER_REQUEST` with exactly one of them.

  `voxsdk_ev_transfer_req_t` gains a trailing `auto_followed` flag, always
...
- **`voxsdk_call_get_info()` — the call metadata every comparable SDK exposes.**
  ...
  It fills a caller-owned `voxsdk_call_info_t` of fixed char arrays, matching
  `voxsdk_audio_device_t`: nothing in it can dangle when the call ends, and
  ...
  is — the peer holding *us*; local hold remains `voxsdk_call_is_held()`.

- **`VOXSDK_REG_RECONNECTING` — a registration the SDK is recovering is no
  longer reported as a failure.** Every transient loss used to arrive as
  `VOXSDK_REG_FAILED`: a REGISTER that timed out on a train, a 5xx, a keepalive
  ...
  - a network handover or a link that dropped entirely, from
    `VOXSDK_NET_CHANGE_DETECTED` (or `NET_DOWN`) until the REGISTER lands on the
    new path.

  `FAILED` is kept for what the SDK has stopped acting on: `VOXSDK_ERR_AUTH`, an
...
- **A network handover left an ICE call with dead audio.** Wi-Fi ↔ cellular
  ...
  live and silent while it runs). `VOXSDK_NET_CALL_ICE_STALE` and
...
- **Outgoing calls could never send their INVITE.** With ICE enabled, baresip
  ...
  invoked. The call then sat in `VOXSDK_CALL_CALLING` indefinitely — zero SIP
  messages on the wire under `trace_sip`, no event of any kind, and
  `voxsdk_call_invite()` had already returned success, so nothing in the SDK
...
- One dial on an IPv4-only network permanently disabled ICE for the whole
  account. `voxsdk_call_invite()` retries with the media-NAT cleared when
...
- `voxsdk_call_invite()` returned success without writing `*out` when
  `ua_connect()` reported success but produced no call, leaving the caller with
  an untouched handle. It now returns `VOXSDK_ERR_INVAL`.

- `voxsdk_call_get_stats()` reported 0 for `mos_lq_min`, `mos_lq_avg`,
...
- **Degraded-link handling** — the SDK already recovered from a *changed*
  ...
  - `cfg.media_stall_ms` (default 4000) fires `VOXSDK_QUALITY_MEDIA_STALL`
    ...
    `VOXSDK_EV_NETWORK` already narrates the outage in more detail.
  - `cfg.rtp_timeout_s` (default 0 = off) wires baresip's `avt.rtp_timeout`,
    ...
    one-way media. Per-call override: `voxsdk_call_set_rtp_timeout()`.
  - `cfg.adaptive_bitrate` steps the Opus encoder down under the loss the
    *peer* reports over RTCP and back up on recovery — halve down, +25% up,
    ...
    `voxsdk_set_adaptive_bitrate()`, `voxsdk_call_set_bitrate()`.
...
- **Keepalive is a real mechanism instead of a dead config field** —
  ...
  `VOXSDK_ERR_TIMEOUT` and, with the new `cfg.keepalive_reregister` (default
  ...
  `voxsdk_account_keepalive_now()` runs the same probe on demand, for a
...
- **`cfg.sip_timer_b_ms` / `cfg.sip_timer_f_ms` now do something** — both were
  ...
  408, surfacing as `VOXSDK_CALL_FAILED` / `VOXSDK_ERR_TIMEOUT`; only
...
- **RFC 3263 SRV failover** — `src/dns.c` implemented the whole NAPTR→SRV chain
  and `vox_dns_resolve()` had no callers, so every retry re-sent to the host
...
- **`cfg.net_ice_handover`** and `VOXSDK_NET_CALL_ICE_STALE` — an ICE call
  ...
  `VOXSDK_ICE_HANDOVER_FAIL_FAST` reports failure after one attempt rather
...
- **Media statistics are on by default** (`cfg.stats_interval_ms` 0 → 2000) and
  ...
  as zero from `voxsdk_call_get_stats()` too — an app that never set it got no
  quality signal anywhere and nothing said so. Everything added above reads
  from this tick. Set it to 0 to opt out.

- A `408` on a call now maps to `VOXSDK_ERR_TIMEOUT` rather than
  `VOXSDK_ERR_INVAL`. It is what the new setup watchdog cancels with, and what
...
- **Python: every config field after `cfg.aec_mode` was written to the wrong
  offset.** `voxsdk_aec_mode_t` is a packed 1-byte enum, and the cffi header
  ...
  `struct_size` check in `voxsdk_init()` never caught it.
  `bindings/python/build.sh` now passes `-DVOXSDK_NO_PACKED_ENUM=1`, selecting
...
- `bindings/flutter`: `VoxSDKConfig.statsIntervalMs` and the three alert
...
- **iOS: init seized the audio session from CallKit** — `voxsdk_init()` called `[AVAudioSession setActive:YES]` unconditionally, so merely starting the SDK (at app launch, or on a PushKit wake while CallKit was still reporting the call) took the exclusive PlayAndRecord route. Apple requires `CXProvider` to be the only activator, in `provider(_:didActivateAudioSession:)`. Activation is now controlled by `platform_audio_activate`; CallKit apps set it to `false` and get category + mode without activation.

- **Global codec list was never marshalled from Flutter** — `VoxSDKConfig.audioCodecs` existed in Dart and stopped there: nothing wrote it into the native config, so a global codec preference was silently ignored (per-account `AccountConfig.audioCodecs` did work). The global list is now a first-class native field and reaches baresip in order.
- **Re-entry on a live stack was unusable** — a host that loses its own runtime while the process survives (Android headless Flutter engine destroying the Dart isolate between push wakeups) came back to a stack that was still up, and `voxsdk_init()` could only answer `VOXSDK_ERR_ALREADY`: the event sink still pointed at the dead runtime and the handles it held were gone. There is now a reattach path — see "Reattaching to a live stack" in `docs/api/overview.md`.

### Added

- `platform_audio_activate` in `voxsdk_config_t` (default `true`) — whether `voxsdk_init()` activates the platform audio session it configures. iOS only; other platforms ignore it. Flutter: `VoxSDKConfig.platformAudioActivate`, normally paired with `VoxSDK.start(manageAudioSession: false)`.
- `audio_codec_names[8][32]` + `audio_codec_name_count` in `voxsdk_config_t` — the global counterpart to the per-account name list, so codecs with no `voxsdk_codec_t` constant (`"g729"`) and any codec a loaded module registers can be selected globally. Precedence: account names → account enums → global names → global enums.
- `voxsdk_is_initialized()` — is the stack up in this process.
- `voxsdk_set_event_handler(cb, userdata, deliver_owned_events)` — re-point event delivery at a new consumer, or park it with `NULL`, without tearing the stack down.
- `voxsdk_account_foreach()`, `voxsdk_account_get_aor()`, `voxsdk_account_get_reg_state()` — re-derive account handles and their state instead of creating duplicates.
- `voxsdk_call_get_account()`, `voxsdk_call_get_state()` — pair with the existing `voxsdk_call_foreach()` to recover a call that arrived while nobody was listening (events are dropped across the gap; the call itself is not).
- Flutter: `VoxSDK.start()` reattaches to a running stack by default (`reattachIfRunning: false` to opt out), `VoxSDK.reattached`, `VoxSDK.accounts` / `VoxSDK.calls`, `VoxSDK.detach()`, `Account.aor`, `Account.regState`.
...
- **Windows DLL — missing exports** — `voxsdk_strerror` and `voxsdk_version` were absent from `voxsdk.def` because the DEF-generation regex required whitespace before the function name, which doesn't match pointer-returning signatures (`const char *fn(`). Fixed regex to accept either whitespace or `*` as the separator.
...
- `rtcp_mux` field in `voxsdk_config_t` (default `true`). Set to `false` to opt out of RTCP-mux and revert to separate RTCP ports (not recommended when ICE is enabled).
...
- `voxsdk_push_provider_t` enum — `VOXSDK_PUSH_PROVIDER_NONE` / `APNS` / `APNS_SANDBOX` / `FCM`.
- Three new fields in `voxsdk_account_config_t`: `push_provider`, `push_token`, `push_param`. When set, the SDK encodes RFC 8599 `pn-provider` / `pn-prid` / `pn-param` URI parameters **inside** the Contact angle brackets on every REGISTER. Self-hosted servers (Kamailio, drachtio, FreeSWITCH) read these from the registrar and use them to wake the device via APNs or FCM.
- `voxsdk_account_set_push_token(acct, token)` — update the push token at runtime without re-creating the account. Re-registers immediately when safe (defers if a REGISTER/UNREGISTER transaction is in flight or a retry backoff is pending). Pass `NULL` to clear all push params.
- `voxsdk_account_add_register_header(acct, name, value)` — attach a custom SIP header to REGISTER requests **only**. Does not appear on INVITE, BYE, REFER, or any other request. Use this for hosted / vendor servers (Twilio, Plivo, Asterisk PJSIP) that dispatch push via non-standard headers rather than RFC 8599.
...
#### Audio recording
- `voxsdk_call_record_start(call, path)` — record call audio to a single mixed WAV file (PCM S16LE). Both the received (RX) and sent (TX) audio are clip-summed into one stream.
- `voxsdk_call_record_stop(call)` — stop recording and finalize the WAV header with correct sizes. The file is also closed automatically if the call is destroyed.
...
- `voxsdk_account_set_retry_policy(acct, initial_ms, max_ms, backoff, max_attempts)` — override the retry policy for a specific account at runtime without recreating it. Overrides the global `reg_retry_*` fields in `voxsdk_config_t` for that account only.
- `voxsdk_account_cancel_retry(acct)` — cancel a pending retry timer and reset the attempt counter. The account stays in `FAILED` state; call `voxsdk_account_register()` to restart manually.
- `voxsdk_account_retry_now(acct)` — skip the current backoff delay and re-register immediately. Resets the attempt counter.
...
- **Race on fast cancel** — if the caller cancels before the event thread delivers INCOMING_CALL, the CLOSED event no longer nulls the call handle first. `voxsdk_call_answer()` no longer spuriously returns `ENOENT`.
...
#### Registration
- **SIP error code parsing** — `strstr(reason, "5")` replaced with proper 3-digit extraction; 415, 451, 486 are no longer misclassified as 5xx. 407 Proxy Auth now correctly maps to `VOXSDK_ERR_AUTH`.
...
#### Memory
- **Call wrapper leak** — wrappers are freed after `VOXSDK_EV_CALL_CLOSED` is delivered. Previously every completed call leaked one wrapper.
...
#### Mutex safety
- **Init-time mutex lock** (UB on Windows/RTOS) — `mtx_lock` before `mtx_init` replaced with `vox_call_global_init()` called from `voxsdk_init`.
- **Shutdown mutex order** — `mtx_destroy` now called after `mtx_unlock` in `vox_call_global_reset`.
...
- **Python** — `_loader.py` now auto-discovers `voxsdk.so` in `dist/<platform>/<arch>/` when running from a source checkout. Manual `LD_LIBRARY_PATH` or file copy no longer required.
- **Rust** — `build.rs` auto-selects the correct `dist/` sub-directory (`linux/x86_64`, `linux/arm64`, `macos/universal`, `windows/x64`) based on the Cargo target. `VOXSDK_LIB_DIR` export no longer required for native builds.
- **Node.js** — `binding.gyp` replaced fragile relative `-L` paths with absolute paths resolved at build time via `node -p`. Sets `-Wl,-rpath` so the addon finds `voxsdk.so` at runtime without `LD_LIBRARY_PATH`. Supports `VOXSDK_DIST_DIR` env var to override.
...
- Static archive merge (`voxsdk.a`) via libtool / lib.exe / ar MRI
- `VOXSDK_SHARED` option for shared library output (`.so` / `.dylib` / `.dll`)
- Platform build scripts: `build-linux.sh`, `build-macos.sh`, `build-ios.sh`, `build-android.sh`, `build-windows.ps1`

#### C++ binding
- Header-only RAII wrapper (`bindings/cpp/voxsdk.hpp`)
...
#### Rust binding
- `VoxSDK-sys` crate (bindgen auto-generated raw FFI)
- `VoxSDK` crate (safe wrapper with `Result` error handling)
...
- `VoxSDK`, `Account`, `Call` Dart classes
- Event delivery via `Stream<VoxSDKEvent>` with `StreamController`
